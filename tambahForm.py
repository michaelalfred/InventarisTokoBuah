...
def submit(id, nama, masuk, kadaluarsa, beli, jual, jumlah):
    cursor = mydb.cursor()
    cursor.execute("INSERT INTO barang (id, nama_item, tanggal_masuk, tanggal_kadaluarsa, harga_beli, harga_jual, jumlah_kg) VALUES (%s, %s, %s, %s, %s, %s, %s)", 
                   (nama, masuk, kadaluarsa, beli, jual, jumlah, id))
    mydb.commit()

def launch():
    tambahFormWindow = Toplevel()

    tambahFormWindow.geometry("578x363")
    tambahFormWindow.configure(bg = "#628469")
    ...
    entry_masuk = DateEntry(tambahFormWindow,
    ...
    entry_kadaluarsa = DateEntry(tambahFormWindow,
                    date_pattern='yyyy-mm-dd',
    )
    ...
    button_2 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: [print("button_2 clicked"), submit(entry_id.get(), entry_nama.get(), entry_masuk.get_date(), entry_kadaluarsa.get_date(), entry_beli.get(), entry_jual.get(), entry_jumlah.get())],
        relief="flat"
    )
    button_2.place(
        x=208.0,
    ...
    tambahFormWindow.resizable(False, False)
    tambahFormWindow.mainloop()
